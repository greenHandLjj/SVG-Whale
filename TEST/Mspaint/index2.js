...
class Path extends Shapes {
    ...
    update(posi = {}) {
        ...
        // }
        // console.log(d[index-1].x2 = posi.x2)
        // d[index-1].y2 = posi.y2
    ...
    endPonitSetPosition(index) {
        ...
        // for(let key of Object.keys(dPoints)){
        //     if(key === 'sign' || key.indexOf('y') >= 0) continue; // 跳过
        //     // x y 赋值给 endPoint x1 y1 赋值给 startControl x2 y2 赋值给 endControl
        //     if(key === 'x') {
        //         //  + (contentLeft - wrapLeft) 这一句是因为前面对坐标点进行了处理，将原点改为了content的左顶点，- 4是为了居中
        //         assitPoints['endPoint'].setAttribute('x', dPoints['x'] + dValue);
        //         assitPoints['endPoint'].setAttribute('y', dPoints['y'] + dValue);
        //     }else if(key === 'x1') {
        //         // startControl 特别注意 - 它在endControl的反方向， x, y 都反
        //         let {reverseX, reverseY} = coordinateReverse({originX: dPoints['x'], originY: dPoints['y'], targetX: dPoints['x2'], targetY: dPoints['y2']});
        //         assitPoints['startControl'].setAttribute('x', reverseX + dValue);
        //         assitPoints['startControl'].setAttribute('y', reverseY + dValue);
        //     }else if(key === 'x2') {
        //         assitPoints['endControl'].setAttribute('x', dPoints['x2'] + dValue);
        //         assitPoints['endControl'].setAttribute('y', dPoints['y2'] + dValue);
        //     }
        // }
...
let rect = new Rect({originX: 100, originY: 100});
rect.update({targetX: 300, targetY: 300});
rect.done();

// let circle = new Circle({originX: 100, originY: 100});
// circle.update({targetX: 300, targetY: 300});
// circle.done();

// let line = new Line({originX: 200, originY: 200});
// line.update({targetX: 400, targetY: 400});
// line.done();

let path = new Path({ originX: 92, originY: 140 });
path.update({ x1: 300, y1: 61 });

path.addNode({ x: 228, y: 110 }); // 添加新节点
// // 属性值缺失
path.update({ x2: 300, y2: 300 });
path.update({ x1: 100, y1: 200 });

path.done();
// path.update({x1:92, y1:140});
// 修改
// path.update({x1:92, y1:140, x2:278, y2:169});
// path.addNode({x:300, y: 110}); // 再次新增节点
// path.update({x1: 120, y1: 120});

// path.stroke('red')


// let path1 = new Path({originX: 180, originY: 180});
// path1.addNode({x:400, y: 200}); // 添加新节点
// path1.update({x1:92, y1:140, x2:278, y2:169});
// path1.stroke('blue')
